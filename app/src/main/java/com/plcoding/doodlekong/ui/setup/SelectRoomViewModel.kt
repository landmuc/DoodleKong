...
class SelectRoomViewModel @Inject constructor(
    ...
    // send onetime events, won't be emitted on state / configuration changes
    // shared flow allows multiple observers (here: 3 fragments)
    private val _setupEvent = MutableSharedFlow<SetupEvent>()
    val setupEvent: SharedFlow<SetupEvent> = _setupEvent

    // will be emitted on every state change / configuration change
